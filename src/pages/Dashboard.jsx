...
export default function Dashboard() {
  ...
  const loadDashboard = useCallback(async () => {
    ...
    const [profileRes, planRes, resumesRes, coverLettersRes, purchasesRes] = await Promise.all([
    ...
    if (profileRes.error || planRes.error || resumesRes.error || coverLettersRes.error || purchasesRes.error) {
  ...
  async function handleCreateCoverLetter() {
    const { data, error } = await supabase
      .from('cover_letters')
      .insert({ user_id: userId, title: 'Untitled Cover Letter' })
      .select('id')
      .single()

    if (error) {
      setErrorMsg('Could not create a new cover letter — please try again.')
      return
    }

    navigate(`/cover-letter/${data.id}/edit`)
  }

  async function handleDuplicateCoverLetter(letterId) {
    const { data: source, error: fetchError } = await supabase
      .from('cover_letters')
      .select('title, content, resume_id, template_id')
      .eq('id', letterId)
      .single()

    if (fetchError || !source) {
      setErrorMsg('Could not duplicate that cover letter — please try again.')
      return
    }
  ...
  async function handleDeleteCoverLetter(letterId, title) {
    const confirmed = window.confirm(`Delete "${title}"? This can't be undone.`)
    if (!confirmed) return

    const { error } = await supabase.from('cover_letters').delete().eq('id', letterId)
    if (error) {
      setErrorMsg('Could not delete that cover letter — please try again.')
      return
    }
    setCoverLetters((prev) => prev.filter((c) => c.id !== letterId))
  }
  ...
  return (
    <div className="page">
      ...
      <div className="section">
        <button className="btn-primary" onClick={handleCreateCoverLetter}>
          + Create Cover Letter
        </button>
      </div>
      ...
      <div className="section">
        ...
        {coverLetters.length === 0 ? (
          <div className="empty-state">
            <p>You haven't created a cover letter yet.</p>
            <button
          ...
          coverLetters.map((letter) => (
            <div className="card resume-card list-card" key={letter.id}>
              <div className="resume-title">{letter.title}</div>
              <div className="resume-meta">Last edited {formatDate(letter.updated_at)}</div>
              <div className="action-grid">
                <button className="action-btn" onClick={() => navigate(`/cover-letter/${letter.id}/edit`)}>
                  Edit
                </button>
                <button className="action-btn" onClick={() => navigate(`/cover-letter/${letter.id}/preview`)}>
                  Preview
                </button>
                <button className="action-btn" onClick={() => handleDuplicateCoverLetter(letter.id)}>
                  Duplicate
                </button>
                <button className="action-btn danger" onClick={() => handleDeleteCoverLetter(letter.id, letter.title)}>
                  Delete
                </button>
              </div>
            </div>
